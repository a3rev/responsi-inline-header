...
define('RESPONSI_IH_VERSION', '1.1.9');
...
if (!defined('RESPONSI_IH_TRAVIS')) {
    if (!file_exists(get_theme_root().'/responsi/functions.php')) {
        return;
    }
    if (!isset($_POST['wp_customize']) && get_option('template') != 'responsi') {
        return;
    }
    if (isset($_POST['wp_customize']) && $_POST['wp_customize'] == 'on' && isset($_POST['theme']) && stristr($_POST['theme'], 'responsi') === false) {
        return;
    }
...
function responsi_ih_upgrade_version()
{
    ...
    if (version_compare(get_option('a3rev_responsi_ih_version'), '1.1.9') === -1) {
        global $responsi_ih;
